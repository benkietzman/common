...
extern "C++"
{
  namespace common
  {
    ...
    // }}}
    // {{{ centralMonitor()
    bool ServiceJunction::centralMonitor(Json *ptRequest, Json *ptResponse, string &strError)
    {
      bool bResult = false;
      list<string> in, out;
      string strJson;
      ...
      ptJson->insert("Service", "centralMonitor");
      if (!m_strApplication.empty())
      {
        ptJson->insert("reqApp", m_strApplication);
      }
      ...
      if (utility()->conf()->m.find("Central Monitor") != utility()->conf()->m.end())
      {
        ptJson->insert("centralMonitorServer", utility()->conf()->m["Central Monitor"]->v);
      }
      in.push_back(ptJson->json(strJson));
      delete ptJson;
      if (request(in, out, strError))
    ...
    bool ServiceJunction::centralMonitor(const string strServer, const string strProcess, Json *ptResponse, string &strError)
    {
      bool bResult = false;
      ...
      bResult = centralMonitor(ptRequest, ptResponse, strError);
      delete ptRequest;
