...
class ModuleJunctionAuth : public Module
{
  ...
  public:
  // {{{ ModuleJunctionAuth()
  ModuleJunctionAuth() : Module(VF_VENDOR, "Allows connecting users to be authenticated against an arbitrary Service Junction request."), pendingExt(this, "junctionauth-wait", ExtensionType::USER)
  {
  }
  // }}}
  // {{{ OnUserRegister()
  ModResult OnUserRegister(LocalUser* user) override
  {
    ...
    size_t unPosition;
    // }}}
    if (user->GetClass()->config->getBool("usejunctionauth", true) && (m_strAllowPattern.empty() || !InspIRCd::Match(user->GetRealUser(), m_strAllowPattern)) && !pendingExt.Get(user))
    {
      std::string strError;
      SSL_CTX *ctx = NULL;
      ...
      if ((ctx = SSL_CTX_new(method)) != NULL)
      {
        ...
        if ((nReturn = getaddrinfo(m_strServer.c_str(), m_strPort.c_str(), &hints, &result)) == 0)
        {
          ...
          if (bConnected[2])
          {
            ...
            {
              strBuffer[1].replace(unPosition, 6, user->GetRealUser());
            }
      ...
      else
      {
        ...
        const char *lcpErrMsg = ERR_error_string(lErrCode, NULL);
        std::stringstream ssError;
  ...
  {
    auto &conf = ServerInstance->Config->ConfValue("junctionauth");
